...
class HuggingfaceEncoderModel(
    ...
    def preprocess(
        ...
        need_offsets = (self.task == MLTask.token_classification) and getattr(self, "return_offsets_mapping", False)

        if need_offsets and not getattr(self._tokenizer, "is_fast", False):
            raise ValueError("return_offsets_mapping requires a fast tokenizer (tokenizers backend).")
        ...
        if self.task == MLTask.sequence_classification:
            ...
            if getattr(self, "return_offsets_mapping", False):
                ...
                if offset_mapping is not None:
                    ...
                    from kserve.utils.utils import from_np_dtype
